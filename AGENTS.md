...
|-----|------|-------------|
| Supabase admin auth | `app/admin/login/_components/LoginForm.tsx`, `app/admin/login/submit/route.ts`, `proxy.ts` (the active gate) + `lib/supabaseServer.ts:shouldForceInsecureCookies` | Admin sign-in uses a native form POST to the server Route Handler which sets cookies on the same 303 redirect response. `proxy.ts` is the active auth gate for this Next/Turbopack setup (exports default function + named `proxy` + `config`; middleware.ts deprecated per build messages). Proxy copies refreshed cookies onto *every* redirect branch. Do not reintroduce browser-client `signInWithPassword` + `router.push`. The `setAll` handler in the submit route must pass Supabase-provided `options` through **unchanged**. All four `setAll` sites (proxy, submit, lib/supabaseServer create*, requireAdmin) must use the shared `shouldForceInsecureCookies` helper. `@supabase/ssr` ≥ 0.12 headers must be forwarded. |
