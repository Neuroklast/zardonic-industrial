...
- **`.supabase.co` is no longer a trusted direct asset host**: `toDirectImageUrl` and the partner-logo canvas pipeline route legacy Supabase Storage URLs through wsrv.nl (single origin fetch, then CDN-cached) instead of every visitor downloading straight from Supabase.
