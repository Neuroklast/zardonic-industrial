...
| 2026-06-23 | grok/admin-login-fix | Grok 4.3 | Setting `disabled={isLoading}` on credential `<input>` fields inside a form's `onSubmit` (for a native `method="POST" action="..."` form) causes the browser to exclude those fields from the submitted payload. Disabled form controls are omitted from `FormData` / enctype body (only the redirect hidden field arrived, length=30). Only disable the submit button; use `readOnly` if field editing must be blocked (readOnly fields are still submitted). | Forms / HTML / React | 🔴 Critical |
| 2026-06-23 | grok/admin-login-fix | Grok 4 | Controlled React inputs (`value` + `onChange` + local state) combined with a native HTML `<form method="POST">` can cause form values to not be reliably present in `request.formData()` on the server Route Handler. For pure native form submissions to API routes, prefer uncontrolled inputs. Also: always guard `signInWithPassword` calls and normalise "missing email or phone" style errors. | DevOps / Architecture | 🟠 High |
...
| 2026-06-23 | grok/build-fix | Grok | In Next.js App Router + Turbopack, any file using `createContext`, `useState`, `useEffect`, `useCallback` etc. at the top level (or exporting hooks) **must** start with `'use client'`. When legacy client components (`CyberpunkOverlay`, overlays, `CookieConsent`, `LocaleContext`, `consent.ts`) are imported (even transitively) by Server Components (`app/page.tsx` → `PublicPageClient`), the build fails hard. Add the directive to the leaf modules containing the APIs, not only the callers. Also audit `'use server'` modules for accidental non-async exports. Add a CI job that runs `build` on PRs. | DevOps | 🔴 Critical |
