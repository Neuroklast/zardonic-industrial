...
Admin uploads (images, video, favicon). URLs resolved through `lib/r2.ts` (`resolveImageUrl`). After a bucket move, leftover `*_url` / `wsrv.nl` hosts are reconciled against the live bucket on **Production deploy** (`instrumentation.ts` → `lib/r2-reconcile-on-deploy.ts`) and from `/admin/data`.
